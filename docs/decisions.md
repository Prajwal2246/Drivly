...
## 022 — Design system: tokens, a few shared components, dark mode opt-in per page (2026-09-19)

**Context:** The UI had no shared design layer: colours, radii and button styles were hand-written in every file. 79 classes used Tailwind shades that don't exist (`text-zinc-650`, `text-zinc-955`, `border-zinc-150`…), so Tailwind generated nothing and those elements silently inherited their parent's colour. Six pages each built their own header (only two had Log out; several used clickable `<div>`s). Twenty `alert()` popups blocked the page. The chosen direction (approved on the style-guide canvas): "warm community", indigo lead colour, light + dark.
**Decision:**
- Semantic tokens in `globals.css` (`--paper`, `--surface`, `--sand`, `--line`, `--ink`, `--ink-muted`, `--primary`, `--success-*`, `--warning-*`, `--danger-*`), exposed to Tailwind via `@theme` as `bg-paper`, `text-ink`, `border-line`, `bg-primary`…, plus `rounded-control`/`rounded-card`/`shadow-raised`. Fonts: Fraunces (headings, `font-display`) + DM Sans (body) via `next/font/google`, replacing Geist.
- Five small components in `src/components/ui/`: `Button` + `buttonClass()` (so `<Link>`s share button styles), `Input`/`Select` (label, hint, error wired with `aria-invalid`/`aria-describedby`), `Badge` (one tone per meaning), `Toaster` + `toast()` (errors stay until dismissed; confirmations fade), `AppHeader` (one header on all signed-in pages, Log out everywhere, `aria-current` on the active link).
- Dark mode values live under `@media (prefers-color-scheme: dark) { .ui-v2 { … } }`: a page opts in by adding `ui-v2` to its root once it's redesigned, so un-migrated hard-coded light pages never render half-dark.
- The 79 invalid shades mapped to the nearest real one, rounding text toward darker so contrast never drops.
**Rejected:** A component library (shadcn/ui, Radix) — six components' worth of need; add one when a real dialog/menu/popover is required. `toast` via React context — a window event is less code and callable from anywhere. Enabling dark mode globally now — every un-migrated page would flip its tokens but keep its hard-coded `bg-white`/`text-zinc-*`, producing unreadable mixes. A `Card` component — `rounded-card border border-line bg-surface` is shorter than its props.
**Consequences:** Until each page is migrated, pages mix the new header/toasts with old styling. When the last page has `ui-v2`, move the dark variables to `:root` and delete the class. `confirm()` for deleting a vehicle stays until the dashboard redesign brings a proper dialog. New UI code should use tokens, never raw `zinc`/`stone` shades.
