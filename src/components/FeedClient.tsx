...
import { Search, Calendar, Clock, DollarSign, LogOut, LayoutDashboard, X, Shield, CheckCircle2, AlertCircle, Loader2, User } from 'lucide-react';
        ...
        <div className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-left">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-zinc-400 font-bold uppercase tracking-wider">{user.society}</span>
            </div>
            <h1 className="text-2xl font-black text-zinc-950 tracking-tight">Available Vehicles in {user.society}</h1>
            <p className="text-sm text-zinc-500 mt-1">Direct community rentals from your verified neighbors.</p>
          </div>
            ...
            {isSuccess ? (
              <div className="p-8 text-center space-y-5">
                ...
                <div className="space-y-1.5">
                  <h3 className="text-xl font-black text-zinc-950 tracking-tight">Request Submitted!</h3>
                  <p className="text-xs text-zinc-500 leading-normal max-w-xs mx-auto">
                    Your request has been logged. We have notified <strong>{selectedVehicle.owner.name}</strong>. You will be able to track this booking in your Dashboard.
                  </p>
                </div>
                <button
                  onClick={() => setSelectedVehicle(null)}
              ...
              <div className="p-6 sm:p-8 space-y-6">
                <div>
                  ...
                  <p className="text-xs text-zinc-400 font-mono mt-0.5">Listed by {selectedVehicle.owner.name}</p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-500">Rental Duration</label>
                  ...
                  <div className="bg-zinc-50 border border-zinc-200/60 rounded-2xl p-4 space-y-2 text-xs">
                    ...
                    <div className="border-t border-zinc-200 pt-2 flex justify-between font-bold text-zinc-900 text-sm">
                      ...
                      <span>₹{selectedVehicle.pricePerHour * bookingHours}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 text-[10px] text-zinc-500 bg-zinc-50 p-3 rounded-xl border border-zinc-200/60">
                  ...
                  <span>Before starting, you must complete a vehicle physical checklist.</span>
                </div>
